...
File.open('/home/hidenba/junk/2018/7/17-204656.html', 'w+') do |file|
  file.puts '<ol style="font-size: 15px;">'
  words.sample(50).each do |word|
    example = []
    first = word[:text].index(word[:target])
    last = first + word[:target].length.pred
    range = (first..last)

    if range.first.nonzero?
      example << word[:text].slice(0..range.first.pred)
    end
    example << '<span style="background: yellow;">'
    example << word[:text].slice(range)
    example << '</span>'

    unless range.last == word[:text].length.pred
      example << word[:text].slice(range.last.next,100)
    end

    file.puts Haml::Engine.new('%li{style: "height: 18px"} #{example.join}').render(Object.new, example: example)
  end
  file.puts '</ol>'
end
